...
0. **인쇄 페이지 규격 절대 보호.** `*_print.html` 및 인쇄 전용 템플릿/CSS의 크기·여백·배치·글꼴·페이지 나눔은 사용자가 명시적으로 인쇄 페이지 수정 요청을 하기 전에는 절대 변경하지 말 것. 전체 화면 레이아웃을 수정할 때도 `@media print` 규칙과 인쇄 템플릿에 영향을 주지 않도록 화면용(`@media screen`) 범위로 제한하고, 인쇄 관련 파일은 건드리지 말 것.
